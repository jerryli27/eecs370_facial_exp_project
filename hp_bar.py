...
class HPBar(pygame.sprite.Sprite):
	...
	def __init__(self, hp_max=PLAYER_HP, pos=GAME_SCREEN_RECT.bottomleft, name='PLAYER'):
		...
		self.pos = pos
		self.bar = pygame.Surface([HP_BAR_FULL_LENGTH, HP_BAR_HEIGHT])
		...
		self.image = pygame.Surface([max(HP_BAR_FULL_LENGTH, hprect.width+self._BUFFER_SIZE),
									 hprect.height+self._BUFFER_SIZE+HP_BAR_HEIGHT])
	...
	def update(self):
		new_length = int(HP_BAR_FULL_LENGTH * (self.hp / self.max_hp))
		self.bar.fill(self._HP_COLOR, rect=Rect(0, 0, new_length, HP_BAR_HEIGHT))
		self.bar.fill(self._EMPTY_COLOR, rect=Rect(new_length, 0, HP_BAR_FULL_LENGTH, HP_BAR_HEIGHT))
		self.hptext = self.get_text_surface()
